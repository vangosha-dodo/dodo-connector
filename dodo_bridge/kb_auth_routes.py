...
async def kb_auth_page(settings: Settings = Depends(settings_dep)) -> HTMLResponse:
    return HTMLResponse(_render_page(settings=settings, result=None))


@router.post("/status")
async def kb_auth_status(
    request: Request,
    settings: Settings = Depends(settings_dep),
) -> Response:
    payload = await _read_payload(request)
    _authorize(settings, payload.get("bridge_key"))
    result = await DodoKbAuthCommandRunner(settings).run("status")
    return _format_result(request, settings, result.data, result.ok, result.error)


@router.post("/refresh")
async def kb_auth_refresh(
    request: Request,
    settings: Settings = Depends(settings_dep),
) -> Response:
    payload = await _read_payload(request)
    _authorize(settings, payload.get("bridge_key"))
    result = await DodoKbAuthCommandRunner(settings).run("refresh")
    return _format_result(request, settings, result.data, result.ok, result.error)
...
def _render_page(settings: Settings, result: dict[str, Any] | None) -> str:
    configured = bool(settings.dodo_kb_auth_helper_command)
    result_html = ""
    ...
    helper = settings.dodo_kb_auth_helper_command or "not configured"
    key_field = ""
...
<head>
  ...
  <title>Dodo Knowledge Base Auth</title>
  <style>
...
<body>
  <main>
    <header>
      <h1>Dodo Knowledge Base authorization</h1>
      <p class="muted">Internal read-only helper for dodopizza.info / knowledgebase.dodois.io sessions.</p>
      <span class="status">Helper: {html.escape(helper)}</span>
    </header>
    <div class="grid">
      <section class="panel">
        <h2>Status</h2>
        <form method="post" action="/auth/kb/status">
          {key_field}
          <button class="secondary" type="submit">Check KB session</button>
        </form>
      </section>
      <section class="panel">
        <h2>Start / refresh</h2>
        <form method="post" action="/auth/kb/refresh">
          {key_field}
          <button type="submit">Refresh KB session</button>
        </form>
        <p class="muted">The helper reads the Dodo email MFA code from the configured OpenClaw mailbox without printing it.</p>
      </section>
    </div>
    {result_html}
  </main>
